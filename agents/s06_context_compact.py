...
def micro_compact(messages: list) -> list:
    ...
    if len(tool_results) <= KEEP_RECENT:
        return messages
    # Find tool_name for each tool_call_id by scanning assistant messages
    tool_name_map = {}
    for msg in messages:
        role = msg["role"] if isinstance(msg, dict) else getattr(msg, "role", None)
        if role == "assistant":
    ...
    for idx in to_clear:
        ...
        if not isinstance(content, str) or len(content) <= 100:
            continue
        tool_id = msg["tool_call_id"] if isinstance(msg, dict) else msg.tool_call_id
        ...
        if isinstance(msg, dict):
            ...
            # ChatCompletionMessage object -- replace in-place via dict
            messages[idx] = {"role": "tool", "tool_call_id": tool_id, "content": f"[Previous: used {tool_name}]"}
    return messages
